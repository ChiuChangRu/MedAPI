...
> **要動程式的人（含 AI 開發代理）請先讀 [`GPT-HANDOVER.md`](GPT-HANDOVER.md)**——
> 部署管線有兩個一動就整站掛掉的地雷，那份文件開頭就講。

本 repo 有三個版本，用途不同：

| | `cloudflare/`（團隊版，推薦） | `docs/`（靜態版） | `app/`（FastAPI 版） |
|---|---|---|---|
| 有沒有網址 | ✅ Cloudflare Workers 免費部署 | ✅ GitHub Pages | ❌ 需自己找主機 |
