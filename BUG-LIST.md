...
## G. 2026-08-09 追加：套用 0 天之後，「待處理」清單最上面還是一堆已歸檔資料

F 節修好了 0 天本身（暫存區確實清空了，畫面也顯示「全部都歸類好了」），但
截圖再回報：清單最上面還是一堆有 📂 資料夾圖示、沒有 🤖 標記的舊記事。

- **根因**：這不是 bug，是設計沒對齊使用者的心智模型。E 節把「收件匣」改成
  「最近作業」時，刻意定義成「不分資料夾、列最後動過的 25 筆」——目的是解決
  「收件匣空了整個面板就消失」。但代價是：使用者剛手動搬移／編輯過某筆
  **已經歸檔好**的記事，它就會佔著清單最上面的位置。天數設定（自動歸類）
  跟這個清單完全是兩套獨立機制，天數再怎麼調都不會讓已歸檔的東西從這裡消失
  ——這正是使用者反覆回報「為什麼還在上面」的根本原因，跟 0 天本身無關。
- **確認方式**：直接用 `AskUserQuestion` 問使用者希望這個清單變成什麼樣，
  三個選項裡選了「只顯示還沒歸檔的」——把清單的定位從「工作記錄」改回
  「代辦事項」。
- **怎麼修**：`GET /entries/recent` 加上 `WHERE`，只回三種記事：
  1. `folder_id IS NULL`（收件匣）
  2. `f.role = 'staging'`（暫存區，含 AI 判斷不出來、`auto_filed_at='failed'`
     而留在原地的）
  3. `auto_filed_at` 有值且不是 `'failed'`（AI 剛自動歸類、使用者還沒確認過
     的——這組即使已經有真正的資料夾也保留在清單裡，不然 🤖 標記／
     confirm-filing 那套審查機制會完全沒有入口，只能靠使用者剛好逛進那個
     資料夾才會發現，等於做了一半的功能）。
  面板名稱從「🕒 最近作業」改回「📥 待處理」，比較準確地反映「這裡列的是
  要你處理的東西」而不是「所有最近動作的紀錄」。空清單的文案也從「還沒有
  任何記事」改成「目前沒有還沒歸檔的東西——太好了！」，避免使用者以為是
  資料庫是空的。

新增測試：`tests/fieldlog-entries-recent-pending.test.js`（5 項，走完整 Worker
驗證 SQL 篩選邏輯）、`tests/fieldlog-days-and-folder-sort.test.js` 補三項前端
接線；`tests/fieldlog-ui-requests-2026-08-07.test.js` 原本鎖「🕒 最近作業」
標題文字的那項改成只鎖「面板存在且永遠可見」這個不變的行為，避免同一件事
分散在兩個檔案各鎖一半。前端版本號 90 → 91。
