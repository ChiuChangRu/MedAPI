...
test("index.html：四欄工具列各自有搜尋、wiki、開發工具三個外部連結、用量區塊", async () => {
  ...
  // 欄二：wiki 連結
  assert.match(html, /href="wiki\.html"/);
...
test("home.css：四欄工具列有響應式斷點，窄螢幕疊成一欄", async () => {
  ...
  assert.match(css, /\.home-toolbar-grid\s*\{[\s\S]*?grid-template-columns:\s*repeat\(4/);
