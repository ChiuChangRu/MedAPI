...
test("v165 強制檢查新 service worker 與說明頁", () => {
  assert.match(app, /register\("sw\.js\?v=165"\)\.then\(\(registration\) => registration\.update\(\)\)/);
  assert.match(index, /help\.html\?v=165/);
  assert.match(sw, /help\.html\?v=165/);
});

test("最新版說明涵蓋資料架構、搬移、垃圾桶與裝置限制", () => {
  for (const text of ["八個工作主目錄", "最多四層", "來源資料夾會消失", "小型載入視窗", "垃圾桶保留 60 天", "AI導用", "v165"]) {
