...
${sections || "<p>尚無任何紀錄或指派。</p>"}
  ...
  // ---- 一次性：昌毅的 5 家廠商匯入（1 家在官方 881 名單、4 家不在改建自訂廠商），
  // 每家都附上他整理的三個現場必問。用固定 id＋內容比對做 idempotent，按幾次都不會
  // 重複新增，測完／團隊都匯入完可以整段刪掉（含前端對應按鈕）----
  const CHANGYI_LONYI_QUESTIONS = "現有導管BOM可涵蓋哪些關鍵原料與規格？\nTip／Heat Forming可製作哪些端型及尺寸公差？\nCDMO是否涵蓋打樣、組裝、檢測至量產服務？";
  const CHANGYI_NEW_VENDORS = [
    {
      id: "custom-changyi-tomind",
      name_zh: "蘇州拓邁醫學",
      name_en: "TOMIND",
      questions: "多層共擠與編織管的最小壁厚及公差為何？\n多腔導管可否以CCD量測彎曲半徑與外觀？\n量產線如何整合自動上料、收料及尺寸監測？",
    },
    {
      id: "custom-changyi-ttmedical",
      name_zh: "東莞天天向上",
      name_en: "TT Medical",
      questions: "球囊導管CDMO是否涵蓋BOM至成品組裝？\n雷射加工與Tip熱成型可處理哪些材料與精度？\n可否提供機器人上下料與3D視覺檢測案例？",
    },
    {
      id: "custom-changyi-huitian",
      name_zh: "天津匯田",
      name_en: "HUITIAN",
      questions: "擠出設備的線速、外徑控制與量測精度為何？\n設備可搭配哪些Tip／Heat Forming製程與管材？\n是否提供CCD彎曲量測及自動收放料整線方案？",
    },
    {
      id: "custom-changyi-tongyao",
      name_zh: "上海通耀醫療",
      name_en: "Shanghai Tongyao Medical",
      questions: "Reflow熱成型的溫度、壓力與時間如何追溯？\n設備可否製作Pigtail端型及複雜彎曲導管？\n可否整合3D視覺、漏氣、拉力及扭矩檢測？",
    },
  ];
  if (path === "/import-changyi-vendors" && method === "POST") {
    const body = await request.json().catch(() => ({}));
    const author = (body.author || "").trim() || "匿名";
    const result = { notes_added: [], notes_skipped: [], vendors_created: [], vendors_skipped: [] };

    async function addQuestionNote(exhibitorId, content) {
      const dup = await db.prepare("SELECT id FROM notes WHERE exhibitor_id = ? AND content = ? AND deleted = 0").bind(exhibitorId, content).first();
      if (dup) { result.notes_skipped.push(exhibitorId); return; }
      await db
        .prepare("INSERT INTO notes (exhibitor_id, author, type, content, created_at) VALUES (?, ?, ?, ?, ?)")
        .bind(exhibitorId, author, "想詢問的問題", content, now())
        .run();
      await logHistory(db, exhibitorId, author, "新增紀錄", `[想詢問的問題] ${content.slice(0, 80)}`);
      result.notes_added.push(exhibitorId);
    }

    // ① 已在官方 881 名單：深圳朗醫科技（Lonyi Medicath，ex-0116）
    await addQuestionNote("ex-0116", CHANGYI_LONYI_QUESTIONS);

    // ② ～⑤ 不在名單，新增自訂廠商後掛問題
    for (const v of CHANGYI_NEW_VENDORS) {
      const existing = await db.prepare("SELECT id FROM custom_exhibitors WHERE id = ?").bind(v.id).first();
      if (existing) {
        result.vendors_skipped.push(v.id);
      } else {
        await db
          .prepare("INSERT INTO custom_exhibitors (id, name_zh, name_en, booth_no, country, category, description, website, added_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
          .bind(v.id, v.name_zh, v.name_en, "", "", "", "", "", author, now())
          .run();
        await logHistory(db, v.id, author, "新增自訂廠商", v.name_zh);
        result.vendors_created.push(v.id);
      }
      await addQuestionNote(v.id, v.questions);
    }
