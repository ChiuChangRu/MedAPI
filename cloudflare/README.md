...
- **展商目錄**：897 家官方真實展商（2026-08-10 重新匯入，見 App 內「📋 資料異動記錄」），靜態 JSON 直接進瀏覽器，篩選零延遲。
...
分享給 9 位同事，第一次進入輸入 PIN + 自己的名字即可開始共筆。
...
## 啟用 LINE 每日摘要（選用，約 5 分鐘）

每天固定時間（台北/上海時間晚上 8 點）自動推播「今天有哪些指派異動、
哪些廠商填了拜訪成果」到你的 LINE，不用打開網站也能掌握進度。

1. 到 [LINE Developers Console](https://developers.line.biz/console/)
   建立一個 **Provider**，底下新增一個 **Messaging API** 頻道（免費）
2. 頻道設定頁面找到：
   - **Channel secret**（在 Basic settings）
   - **Channel access token**（在 Messaging API 分頁，按 Issue 產生長期 token）
3. 到 Cloudflare Worker 的 Settings → Variables and Secrets，新增兩個 **Secret**：
   - `LINE_CHANNEL_SECRET`
   - `LINE_CHANNEL_ACCESS_TOKEN`
4. 回到 LINE Developers 頻道的 Messaging API 分頁，**Webhook URL** 填：
   `https://你的網址.workers.dev/line/webhook`，並開啟 **Use webhook**
5. 用手機掃頻道頁面的 QR Code 加這個官方帳號好友——加好友當下會自動
   收到一則確認訊息，代表已加入每日摘要的推播名單
6. 想收摘要的人都各自加好友即可，不限一人

摘要內容：當天「負責人指派異動」與「拜訪成果儲存」兩類紀錄，
其他瀏覽、篩選、口袋名單等操作不會列入（避免每天收到一長串雜訊）。

## 啟用錄音轉文字（選用，約 2 分鐘）

展商詳情頁上傳的錄音檔，可以按「轉文字」自動辨識成文字（用
Cloudflare Workers AI 的語音辨識模型），免費額度每天 10,000
Neurons，9 人團隊展期用量通常用不完。

1. 打開 `cloudflare/wrangler.jsonc`，把最下面 `"ai": { "binding": "AI" }`
   那行的註解拿掉，commit + push
2. 部署完成後重新整理網站，展商詳情頁的錄音附件旁就會出現「轉文字」按鈕

> 準確度僅供參考：展場現場環境吵雜、專有名詞（零件型號、廠牌名稱）
> 辨識未必準確，建議當作可搜尋的重點摘要，不要當逐字稿引用。
...
| `line_recipients` | 加過 LINE 官方帳號好友、要收每日摘要的使用者 |
