...
- **隨身記（fieldlog）**——個人現場採集：參展／拜訪／實驗的逐字稿、照片文字、
  ...
  就能列目錄的工具；也含 2026-07-26 一次性匯入的 LitDB 文獻/專利
  （152 筆親水塗層／活檢針機構／醫材包裝資料，只有文字，不含 PDF）
- **Medtec 2026 參展系統**——585 家展商名單＋團隊拜訪紀錄＋附件全文

**預設唯讀。** 17 個工具裡 15 個只做 SELECT／fetch，唯二例外是
...
5. 認證方式若有選項，選「No authentication」或「None」——PIN 已經包在網址裡了，
   不需要再另外設定認證
6. 儲存後，ChatGPT 應該會呼叫一次 `tools/list` 抓到下面這 17 個工具；
   如果連線失敗，先確認網址結尾的 PIN 有沒有貼對、貼完整

設定好之後，直接在對話裡問就好，例如：「幫我查展商裡做親水塗層的」
「上次實驗紀錄裡提到的固化溫度是多少」「wiki 的抗結痂條目現在寫到哪」
「litdb 裡有沒有講活檢針擊發機構的文獻」（LitDB 已一次性匯入隨身記，
`search_fieldlog` 就查得到）——GPT 會自己判斷該呼叫哪個工具。
...
| 工具 | 用途 |
|---|---|
| `search_fieldlog` | 搜紀錄標題／內文／欄位＋附件檔名／逐字稿／擷取文字；可用 folder_id／folder_type 縮小範圍；也涵蓋一次性匯入的 LitDB 文獻/專利（「LitDB 文獻庫」資料夾，152 筆親水塗層／活檢針機構／醫材包裝資料，2026-07-26） |
...
- 三個後端來源（策略地圖 Wiki、隨身記、Medtec）各自獨立的 Cloudflare
  ...
  LitDB（`chiuchangru/litdb`）已於 2026-07-26 一次性匯入隨身記，之後不再
  對外部的 litdb repo 做任何查詢
