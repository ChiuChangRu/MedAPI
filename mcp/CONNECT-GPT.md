...
- **隨身記（fieldlog）**——個人現場採集：參展／拜訪／實驗的逐字稿、照片文字、
  現在還有記事與記事之間的交叉關聯（引用標準、對照廠商產品…）
- **Medtec 2026 參展系統**——585 家展商名單＋團隊拜訪紀錄＋附件全文

**預設唯讀。** 14 個工具裡 12 個只做 SELECT／fetch，唯二例外是
...
5. 認證方式若有選項，選「No authentication」或「None」——PIN 已經包在網址裡了，
   不需要再另外設定認證
6. 儲存後，ChatGPT 應該會呼叫一次 `tools/list` 抓到下面這 14 個工具；
...
| `get_fieldlog_attachment` | 拉單一附件「完整、未截斷」的全文——附件內容被截斷時用這個 |
...
- **預設唯讀**：程式碼裡絕大多數是 SELECT／fetch；`create_fieldlog_entry`／
  ...
  改掉或刪掉既有的任何一筆資料
- 三個後端系統（策略地圖 Wiki、隨身記、Medtec）各自獨立的 Worker／資料庫，
  這個 MCP 只是加一層查詢介面，不做跨系統的資料庫合併
