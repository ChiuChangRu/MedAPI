    ...
    try {
      ...
      await step.do('update-db-status', async () => {
        ...
        await this.env.DB.prepare(
          `UPDATE attachments SET embedding_status = 'done', vector_id = ? WHERE id = ?`
        ).bind(upsertResult.vectorId, attachmentId).run();
        console.log(`[DB] 更新 attachment ${attachmentId} 的 embedding 狀態為 done`);
        return { success: true };
      });

      return { success: true, attachmentId, vectorId: upsertResult.vectorId, textLength: embeddingVector.textLength };
    } catch (error) {
      ...
      return { success: false, attachmentId, error: error.message };
    }
  }
}
...
async function transcribeAttachment(env, db, old) {
  ...
  await triggerEmbedding(env, { attachmentId: old.id, textContent: text, entryId: old.entry_id, type: old.kind });
  return text;
}
...
async function handleApi(request, env, url) {
  ...
  if (path === "/search" && method === "GET") {
    ...
    if (!query.trim()) return bad("查詢詞不可為空");
        ...
        try {
          ...
          if (folderId) {
            const entryRow = await db.prepare(
              `SELECT e.folder_id FROM entries e, attachments a WHERE a.id = ? AND a.entry_id = e.id`
            ).bind(attachmentId).first();
            if (!entryRow || entryRow.folder_id !== Number(folderId)) continue;
          }

          const att = await db.prepare("SELECT * FROM attachments WHERE id = ?").bind(attachmentId).first();
          if (!att) continue;

          const entry = await db.prepare("SELECT * FROM entries WHERE id = ?").bind(att.entry_id).first();

          results.push({ score: match.score, attachment: att, entry: entry });
        } catch (err) {
          continue;
        }
      }
  ...
  if (path === "/batch/ocr" && method === "POST") {
    ...
    for (const att of pending) {
      ...
      try {
        ...
        await triggerEmbedding(env, { attachmentId: att.id, textContent: text, entryId: att.entry_id, type: att.kind });
  ...
  if (ocrMatch && method === "POST") {
    ...
    if (nativeKind) {
      ...
      await triggerEmbedding(env, { attachmentId: id, textContent: text, entryId: old.entry_id, type: old.kind });
    ...
    if (isPdf) {
      ...
      await triggerEmbedding(env, { attachmentId: id, textContent: pdfText, entryId: old.entry_id, type: old.kind });
