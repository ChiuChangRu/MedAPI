...
const PDF_EDITOR_BOOTSTRAP = String.raw`
;(() => {
  if (window.__fieldlogPdfEditorV38) return;
  window.__fieldlogPdfEditorV38 = true;
  ...
  async function savePdf() {
    ...
    try {
      ...
      closeEditor();
      if (typeof openEntry === "function") await openEntry(state.entryId);
    } catch (error) {
  ...
  if (originalAttHtml) {
    attHtml = function fieldlogV38AttHtml(attachment, siblings) {
  ...
  if (originalBindAttActions) {
    bindAttActions = function fieldlogV38BindAttActions(entryId) {
