...
2. 有空時打開紀錄：轉文字、補欄位、移動到資料夾（四層任一層都可以，
   已經歸過檔的也能再搬）。v142 B 模式每天台灣時間 02:00 只處理待分類中
   新增或內容更新過的記事：人工核准規則優先，否則用 BGE-M3 語意相似結果
   與 Cloudflare Workers AI 的 Llama 交叉判定；高信心一致才自動搬，中等信心
   只顯示「套用／忽略」，判斷不足則原地保留。自動搬動可復原，也不會建立、
   刪除或改名資料夾。**每日排程永遠不會重新整理已在正式資料夾的內容**；若要
   整理既有母體，只能明確呼叫一次性後臺命令，且每筆只評估一次。資料夾清單
   （首頁與每一層子資料夾）都能切「🔤 名稱排序 / 🆕 新到舊」，一個開關
   同時套用在所有層級
...
| `entries` | 紀錄（folder_id 空＝收件匣；`auto_filed_at`／`auto_filed_reason`＝B 模式自動搬動的待確認標記） |
| `attachments` | 照片/錄音段/檔案（R2），offset_secs＝錄音時間點 |
| `history` | append-only 歷程 |
| `filing_suggestions` | B 模式最新判定、信心、內容版本與人工確認／忽略／復原狀態 |

既有母體的一次性整理端點：`POST /api/admin/review-existing-filing?limit=25`。
它不在前臺顯示，也沒有掛入 cron；需明確呼叫，重複呼叫只會接著處理尚未評估的
下一批。只有高信心結果會搬，原資料夾保留供復原。
...
- 右欄預覽（錄音記事與 AI 整理筆記）明確指定字級：內文 14px、標題 14–15px、
  輔助說明 12px。之前 `body` 沒設字級，沒指定的元素一律繼承 16px，跟 App 其他
  地方（12–14px）比起來忽大忽小。間距一併收緊。
- AI 整理筆記沒內容時，「尚未整理」只在標題下顯示一次（原本出現兩次）。
