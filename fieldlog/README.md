...
2. 有空時打開紀錄：轉文字、補欄位、移動到資料夾（四層任一層都可以，
   已經歸過檔的也能再搬）。暫存區裡放滿設定天數沒人動的，排程會用 AI 從
   **現有資料夾**裡挑一個歸進去，並在記事上標 🤖，點一下可以確認或改掉。
   天數不是寫死的規則——首頁「🕒 最近作業」下方那一行有輸入框，自己就能
   調（0–30 天，改了立刻生效，不用重新部署；填 0 代表不等待，暫存區的東西
   下次排程或按「現在就跑一次」就全部立即歸檔；沒設定過時退回 Worker 變數
   `AUTO_FILE_DAYS`，都沒設就用預設值 4 天）。資料夾清單（首頁與每一層
   子資料夾）都能切「🔤 名稱排序 / 🆕 新到舊」，一個開關同時套用在所有層級
