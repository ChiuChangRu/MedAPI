...
2. 有空時打開紀錄：轉文字、補欄位、移動到資料夾（四層任一層都可以，
   已經歸過檔的也能再搬）。暫存區純手動，沒有天數門檻或 AI 自動歸類——
   自己找時間把東西搬去該去的資料夾就好（這個機制 2026-08-09 拿掉了；
   舊資料上如果還留著 🤖 標記，點一下一樣能確認或改掉）。資料夾清單
...
| `entries` | 紀錄（folder_id 空＝收件匣；`auto_filed_at`／`auto_filed_reason`＝已移除的 AI 自動歸類功能留下的歷史標記，不會再有新資料） |
