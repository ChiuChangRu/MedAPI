...
const RECENT_FOLDER_STORAGE_KEY = "fieldlog_recent_folder_ids";
const RECENT_FOLDER_LIMIT = 5;

/**
 * 最近使用目錄是介面捷徑，不是文件內容，因此存在瀏覽器即可，不替資料庫增加一張
 * 只服務首頁的表。只存 id、不存名稱或路徑：資料夾改名或搬家後，首頁會用目前的
 * FOLDERS 重新算出正確顯示；已刪除／已無權限的 id 也會在 render 時剔除。
 */
function readRecentFolderIds() {
  try {
    const parsed = JSON.parse(localStorage.getItem(RECENT_FOLDER_STORAGE_KEY) || "[]");
...
function saveRecentFolderIds(ids) {
  try {
    localStorage.setItem(RECENT_FOLDER_STORAGE_KEY, JSON.stringify(ids.slice(0, RECENT_FOLDER_LIMIT)));
  } catch {
    // 無痕模式或瀏覽器停用儲存時，捷徑不應阻斷資料夾本身的開啟。
  }
}

function recentFolders() {
  const storedIds = readRecentFolderIds();
  const byId = new Map(FOLDERS.map((folder) => [Number(folder.id), folder]));
  const folders = storedIds.map((id) => byId.get(id)).filter(Boolean);
  const validIds = folders.map((folder) => Number(folder.id));
  if (validIds.length !== storedIds.length) saveRecentFolderIds(validIds);
  return folders;
}

function renderRecentFolders() {
  const wrap = $("recent-folder-list");
  if (!wrap) return;
  const folders = recentFolders();
  if (!folders.length) {
    wrap.innerHTML = `<p class="sub">尚無最近使用的目錄。</p>`;
    return;
  }
  wrap.innerHTML = folders.map((folder) => {
    const path = folderDisplayPath(folder);
    const section = FOLDER_CATEGORY_META[folderSectionKey(folder)] || FOLDER_CATEGORY_META.misc;
    const parentPath = path.slice(0, -1).join("／") || section.label;
    return `<button class="recent-folder-card" type="button" data-id="${folder.id}"
      title="開啟 ${esc(path.join("／"))}"
      style="--recent-accent:${section.accent};--recent-bg:${section.bg}">
      <span class="recent-folder-icon" aria-hidden="true">${section.icon}</span>
      <span class="recent-folder-copy">
        <strong>${esc(folder.name)}</strong>
        <small>${esc(parentPath)}</small>
      </span>
    </button>`;
  }).join("");
  wrap.querySelectorAll(".recent-folder-card").forEach((button) => {
    button.onclick = () => openFolder(Number(button.dataset.id));
  });
}

function rememberRecentFolder(id) {
  const numericId = Number(id);
  if (!FOLDERS.some((folder) => Number(folder.id) === numericId)) return;
  const nextIds = [numericId, ...readRecentFolderIds().filter((storedId) => storedId !== numericId)]
    .slice(0, RECENT_FOLDER_LIMIT);
  saveRecentFolderIds(nextIds);
  renderRecentFolders();
}
...
function recordingStatus(audioAttachments) {
  ...
  const countDetail = `已轉錄 ${completedCount} 段｜未轉錄 ${remainingCount} 段`;
  ...
  if (audio.some((item) => item.transcribed_at === "processing")) {
    return { tone: "working", label: "轉錄中", detail: `${countDetail}｜正在建立逐字稿`, ...progress };
  }
  if (audio.some((item) => item.transcribed_at === "auto_failed")) {
    return { tone: "failed", label: "部分轉錄失敗", detail: `${countDetail}｜隔日排程會接續；也可手動重試失敗段落`, ...progress };
  }
  if (audio.every((item) => item.transcribed_at === "skipped")) {
    return { tone: "muted", label: "未啟用轉錄", detail: `${countDetail}｜這份錄音目前設為不轉錄`, ...progress };
  }
  if (audio.every((item) => item.transcribed_at)) {
    return transcriptChars
      ? { tone: "done", label: "已全部轉錄", detail: `${countDetail}${doneDetail ? `｜${doneDetail}` : ""}`, ...progress }
      : { tone: "done", label: "已全部轉錄", detail: `${countDetail}｜沒有辨識到可轉成文字的語音`, ...progress };
  }
  if (completedCount) {
    return { tone: "pending", label: "等待自動轉錄", detail: `${countDetail}｜額度恢復後自動接續`, ...progress };
  }
  return { tone: "pending", label: "等待自動轉錄", detail: `${countDetail}｜額度恢復後會依錄音時間自動處理`, ...progress };
}
...
async function openFolder(id) {
  ...
  // API 成功回來才算「使用過」；網路失敗或 id 已不存在時不能污染最近清單。
  rememberRecentFolder(id);
...
function aiNoteStatusLabel(note) {
  const labels = {
    waiting_transcription: "等待逐字稿完成",
    pending: "等待 Claude 整理",
    failed: "整理失敗，等待重試",
    completed: "AI 已整理",
    manual: "人工已修改",
  };
...
function bindAiNoteFileInput(prefix) {
  const picker = $(`${prefix}-ai-note-file`);
  const textarea = $(`${prefix}-ai-note`);
  if (!picker || !textarea) return;
  picker.onchange = async () => {
    const file = picker.files?.[0];
    picker.value = "";
    if (!file) return;
    if (!/\.md$/i.test(file.name) && !/^(text\/markdown|text\/plain)/i.test(file.type || "")) {
      showToast("只接受 Markdown（.md）檔案");
      return;
    }
...
async function renderEntryEditor(entryId) {
  ...
  let initialAiNoteMarkdown = String(entry.ai_note?.markdown || "");
  ...
  $("folder-preview-manage").disabled = !recordingAudio.length;
  $("folder-preview-manage").onclick = recordingAudio.length
    ? () => openRecordingActions(entryId).catch((error) => showToast("開啟錄音操作失敗：" + error.message))
    : null;
  $("entry-preview-editor").onsubmit = async (event) => {
    event.preventDefault();
    const automatic = !!event.automatic;
    ...
    try {
      ...
      if (recordingAudio.length && aiMarkdown !== initialAiNoteMarkdown) {
        ...
        initialAiNoteMarkdown = aiMarkdown;
      }
      if (!automatic) showToast("記事已儲存");
...
async function showRecordingPreview(entryId) {
  // 錄音也是一份 Word 文件，不再先進另一個唯讀「呈現模式」。原始音訊、照片、
  // 重轉錄與刪除集中在右上角 ⋯，正文只在這個共用編輯器出現一次。
  return usesDesktopRightPane() ? showEntryEditor(entryId) : openEntry(entryId);
}
...
async function renderFilePreview({ entryId, attachmentId, filename, key, mime, kind }) {
  ...
  const modernOffice = ["docx", "xlsx"].includes(ext);
  if (image) body.innerHTML = `<img class="folder-preview-image" src="${url}" alt="${esc(filename)}" />`;
    ...
    const { renderPptxPreview } = await import("./pptx-preview.js?v=182");
    await renderPptxPreview(url, body, filename);
  }
...
async function renderFileEditor(entryId, attachmentId) {
  ...
  const restoreSavedFile = () => showFileEditor(entryId, attachmentId)
    .catch((error) => showToast("還原失敗：" + error.message));
  $("folder-preview-open").hidden = false;
  ...
  $("folder-preview-edit").hidden = false;
  $("folder-preview-edit").textContent = "還原";
  $("folder-preview-edit").onclick = restoreSavedFile;
  $("folder-preview-save").hidden = false;
...
async function openEntry(id) {
  ...
  const entryAudio = (e.attachments || []).filter((a) => a.kind === "audio" && !a.source_pdf_id);
  let initialAiNoteMarkdown = String(e.ai_note?.markdown || "");
...
async function loadRecordingEditor(entryId) {
  return openRecordingEditor(entryId);
}

async function openRecordingManager(entryId) {
  // 舊呼叫點的相容別名。不可再把右欄換成第二套「錄音管理編輯器」；⋯ 只開
  // 操作選單，關掉後底下仍是同一份 Word 文件。
  return openRecordingActions(entryId);
}

async function renderRecordingManager(entryId, entry, audio) {
  if ($("entry-overlay")?.classList.contains("open")) closeEntry();
  const photos = (entry.attachments || []).filter((item) => item.kind === "photo" && !item.source_pdf_id);
  const body = $("folder-preview-body");
  setFolderPreviewTitle(`管理｜${entry.title || "錄音"}`);
  clearFolderPreviewEditorToolbar();
  body.innerHTML = `<div class="recording-editor preview-editor">
    <section class="preview-management-panel recording-editor-management">
      <strong>原始錄音（${audio.length} 段）</strong>
      <p class="sub">正文統一在 Word 文件編輯；此處只管理原始資料與重新整理。</p>
      <div class="recording-editor-audio">${audio.map((item, index) => `<section>
        ...
        <audio controls preload="metadata" src="${fileUrlForKey(item.key)}"></audio>
        <div><a class="recording-download" href="${fileUrlForKey(item.key)}" download="${esc(item.filename)}">下載原始錄音</a>
        <button class="btn small danger recording-manager-audio-delete" type="button" data-audio-id="${item.id}" data-filename="${esc(item.filename)}">刪除此段</button></div>
      </section>`).join("")}</div>
    </section>
    ${photos.length ? `<section class="preview-management-panel"><strong>原始照片（${photos.length} 張）</strong>
      <div class="recording-editor-photos">${photos.map((item) => `<a href="${fileUrlForKey(item.key)}" download="${esc(item.filename)}"><img src="${fileUrlForKey(item.key)}" alt="${esc(item.filename)}"><span>${esc(item.filename)}｜${esc(fmtSecs(item.offset_secs || 0))}</span></a>`).join("")}</div>
    </section>` : ""}
    <section class="preview-management-panel"><strong>資料包操作</strong><div class="preview-management-actions">
      <button class="btn small" id="recording-manager-compose" type="button">依時間軸整理圖文</button>
      <button class="btn small" id="recording-manager-transcribe" type="button">重新轉錄全部</button>
      <button class="btn small" id="recording-manager-move" type="button">移動資料包</button>
      <button class="btn small danger" id="recording-manager-delete" type="button">刪除資料包</button>
    </div></section>
  </div>`;
  $("folder-preview-open").hidden = true;
  $("folder-preview-save").hidden = true;
  $("folder-preview-edit").hidden = false;
  $("folder-preview-edit").textContent = "返回文件";
  $("folder-preview-edit").onclick = () => showEntryEditor(entryId).catch((error) => showToast("返回文件失敗：" + error.message));
  $("folder-preview-manage").disabled = true;
  $("folder-preview-manage").onclick = null;

  $("recording-manager-compose").onclick = async () => {
    const button = $("recording-manager-compose");
    button.disabled = true;
    try {
      const result = await api(`/entries/${entryId}/compose-timeline`, { method: "POST", body: "{}" });
      if (result.updated) showToast("已依錄音時間軸整理圖文");
      else if (result.reason === "manual_body") showToast("文件已有人工修改，已保留原文，沒有覆蓋");
      else showToast("目前沒有可整理的逐字稿或照片");
      await showEntryEditor(entryId);
    } catch (error) { showToast("整理圖文失敗：" + error.message); button.disabled = false; }
  };
  $("recording-manager-transcribe").onclick = async () => {
    if (!confirm(`重新轉錄 ${audio.length} 段錄音？\n\n會使用 Cloudflare AI 額度；人工修改過的 Word 文件不會被覆蓋。`)) return;
    const button = $("recording-manager-transcribe");
    button.disabled = true;
    try {
      ...
      showToast("錄音已重新轉錄並整理");
      await refreshFolderView();
      await showEntryEditor(entryId);
    } catch (error) { showToast("重新轉錄失敗：" + error.message); button.disabled = false; button.textContent = "重新轉錄全部"; }
  };
  $("recording-manager-move").onclick = async () => {
    const picked = await openFolderPicker({ title: "移動錄音資料包", desc: `把「${entry.title || "錄音"}」連同音訊、逐字稿及照片一起移到哪裡？`, currentId: entry.folder_id || null, allowInbox: true });
    if (!picked) return;
    try {
      if (picked.id === null) await api(`/entries/${entryId}`, { method: "PUT", body: JSON.stringify({ folder_id: null }) });
      else await moveInboxEntry(entryId, picked.id, entry.folder_id || null);
      showToast("錄音資料包已移動"); await refreshFolderView(); clearFilePreview("錄音資料包已移動，請重新選取。");
    } catch (error) { showToast("移動失敗：" + error.message); }
  };
  $("recording-manager-delete").onclick = async () => {
    if (!confirm(`將「${entry.title || "錄音"}」整個資料包移到垃圾桶？\n\n音訊、逐字稿與照片會一起移入，保留 60 天。`)) return;
    try { await api(`/entries/${entryId}`, { method: "DELETE" }); showToast("錄音資料包已移到垃圾桶"); await refreshFolderView(); clearFilePreview("錄音資料包已移到垃圾桶。"); }
    catch (error) { showToast("刪除失敗：" + error.message); }
  };
  body.querySelectorAll(".recording-manager-audio-delete").forEach((button) => {
    button.onclick = async () => {
      if (!confirm(`確定刪除「${button.dataset.filename || "這段錄音"}」？此操作無法復原。`)) return;
      button.disabled = true;
      try { await api(`/attachments/${button.dataset.audioId}`, { method: "DELETE" }); showToast("已刪除錄音段"); await refreshFolderView(); await openRecordingManager(entryId); }
      catch (error) { showToast("刪除錄音失敗：" + error.message); button.disabled = false; }
    };
  });
}
...
async function openRecordingActions(entryId) {
  ...
  modal.innerHTML = `
    ...
    <div class="recording-action-list">
      ...
      <section class="recording-download-list"><h3>⬇️ 原始錄音</h3>${audio.map((item, index) => `<div class="recording-action-audio-row">
        <a href="${fileUrlForKey(item.key)}" download="${esc(item.filename)}">${audio.length > 1 ? `第 ${index + 1} 段｜` : ""}${esc(item.filename)} <small>${esc(fmtBytes(item.size))}</small></a>
        <button class="btn small danger recording-action-audio-delete" type="button" data-audio-id="${item.id}" data-filename="${esc(item.filename)}">刪除此段</button>
      </div>`).join("")}</section>
  ...
  $("recording-action-edit").onclick = () => {
    closeEntry();
    openRecordingEditor(entryId).catch((error) => showToast("開啟編輯失敗：" + error.message));
  };
  $("recording-action-compose").onclick = async () => {
    const button = $("recording-action-compose");
    button.disabled = true;
    try {
      const result = await api(`/entries/${entryId}/compose-timeline`, { method: "POST", body: "{}" });
      if (result.updated) showToast("已依錄音時間軸整理圖文");
      else if (result.reason === "manual_body") showToast("文件已有人工修改，已保留原文");
      else showToast("目前沒有可整理的逐字稿或照片");
      closeEntry();
      await refreshFolderView();
      await openRecordingEditor(entryId);
    } catch (error) {
      showToast("整理圖文失敗：" + error.message);
      button.disabled = false;
    }
  };
  ...
  modal.querySelectorAll(".recording-action-audio-delete").forEach((button) => {
    button.onclick = async () => {
      ...
      try {
        ...
        await refreshFolderView();
        const fresh = await api(`/entries/${entryId}`);
        const remaining = (fresh.attachments || []).filter((item) => item.kind === "audio" && !item.source_pdf_id);
        if (remaining.length) await openRecordingActions(entryId);
        else await openRecordingEditor(entryId);
      } catch (error) {
        showToast("刪除錄音失敗：" + error.message);
        button.disabled = false;
      }
